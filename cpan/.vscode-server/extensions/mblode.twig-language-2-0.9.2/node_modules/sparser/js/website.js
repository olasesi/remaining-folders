...
(function website() {
    "use strict";
    const svg_left = document.getElementById("svg_left"), svg_right = document.getElementById("svg_right");
    if (window.innerWidth > 800) {
        let width = (window.innerWidth / 10), height = (window.innerHeight / 10), a = 0;
        const blob = document.getElementById("blobs"), screen = blob.getElementsByTagName("div")[0], style = document.createElement("style"), blobs = 8, ff = (navigator.userAgent.indexOf("Gecko/") > 0), size = width / 4, content = document.getElementById("content"), resize = function website_resize() {
            let styletext = style.innerHTML, wscale = ((window.innerWidth / 10) - 100) / 12.5;
        ...
        }, createBlob = function website_createBlob() {
            const rand = [Math.floor(Math.random() * 256), Math.floor(Math.random() * 256), Math.floor(Math.random() * 256)], div = document.createElement("div"), inner = document.createElement("span"), randa = Math.floor(Math.random() * 100), randb = Math.floor(Math.random() * 100);
            let back = [rand[0].toString(16), rand[1].toString(16), rand[2].toString(16)];
            if (back[0].length < 2) {
                back[0] = `0${back[0]}`;
            }
            if (back[1].length < 2) {
