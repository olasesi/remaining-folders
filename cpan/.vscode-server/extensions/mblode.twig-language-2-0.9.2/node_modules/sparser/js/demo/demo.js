...
window.sparser=sparser;/*jslint browser:true */
/*eslint-env browser*/
(function web() {
    
    let editor, options = window.sparser.options, lang = ["", "", ""], language = options.language, lexer = options.lexer;
    const sparser = window.sparser, def = sparser.libs.optionDef, acetest = (location.href.toLowerCase().indexOf("ace=false") < 0 && typeof ace === "object"), aceControl = document.getElementById("aceControl"), input = document.getElementById("input"), dataarea = document.getElementById("data"), datatext = document.getElementById("datatext"), element_language = document.getElementById("option-language"), element_lexer = document.getElementById("option-lexer"), textsize = (navigator.userAgent.indexOf("like Gecko") < 0 && navigator.userAgent.indexOf("Gecko") > 0 && navigator.userAgent.indexOf("Firefox") > 0)
        ? 13
        : 13.33, 
    // eliminate page navigation by clicking the backspace key
    backspace = function web_backspace(event) {
        const e = event || window.event, f = e.srcElement || e.target;
        if (e.keyCode === 8 && f.nodeName !== "textarea" && ((f.nodeName === "input" && f.type !== "text") || f.nodeName !== "input")) {
            e.preventDefault();
            return false;
        }
    }, saveOptions = function web_saveOptions() {
    ...
    handler = function web_handler() {
        let output, startTime = 0;
        const value = (acetest === true)
            ? editor.getValue()
            : input.value, startTotal = Math.round(performance.now() * 1000), builder = function web_handler_builder() {
            let a = 0, body = document.createElement("thead");
            const len = window.sparser.parse.count + 1, table = document.createElement("table"), cell = function web_handler_builder_cell(data) {
        ...
        }, lexVal = element_lexer.value.replace(/\s+/g, ""), langVal = element_language.value.replace(/\s+/g, "");
        ...
        (function web_handler_perfParse() {
            const endTime = Math.round(performance.now() * 1000), time = (endTime - startTime) / 1000;
        ...
        (function web_handler_perfTotal() {
            const endTime = Math.round(performance.now() * 1000), time = (endTime - startTotal) / 1000;
            document.getElementById("timetotal").getElementsByTagName("span")[0].innerHTML = time + " milliseconds.";
        }());
    }, 
    // toggle display of options
    toggleOptions = function web_toggleOptions() {
        const toggle = document.getElementById("toggle-options"), opts = document.getElementById("options");
    ...
    optControls = function web_optControls(event) {
        const node = event.srcElement || event.target, sel = (node.nodeName.toLowerCase() === "select")
            ? node
            : null, input = (node.nodeName.toLowerCase() === "input")
            ? node
            : null, id = node.getAttribute("id").replace(/option-(((false)|(true))-)?/, ""), lex = def[id].lexer, type = def[id].type, lexlen = lex.length, value = (sel === null)
            ? input.value
        ...
        if (id === "format") {
            const data = document.getElementById("data"), dataparent = data.parentNode, text = document.getElementById("datatext"), textparent = text.parentNode;
            if (value === "html") {
    ...
    height = {
        ...
        html: function web_height_html() {
            dataarea.style.height = height.math(10, 0);
        },
        math: function web_height_math(scale, offset) {
            const ff = (textsize === 13) // variability for Gecko
                ? (scale === 10) // pull the output down to align to input
                    ...
                    : (offset < -4) // pull input down to eliminate a white bar
                        ...
                        : -0.2
                : 0;
            let heightn = Math.max(document.documentElement.scrollHeight, document.getElementsByTagName("body")[0].scrollHeight);
    ...
    window.onerror = function web_onerror(msg, source) {
        document.getElementById("errors").getElementsByTagName("span")[0].innerHTML = msg + " " + source;
    };
    { // set option defaults and event handlers
        const select = document.getElementsByTagName("select"), input = document.getElementsByTagName("input"), lexkeys = Object.keys(options.lexer_options), lexlen = lexkeys.length, inputValues = function web_inputValues(node) {
            const name = node.getAttribute("id").replace(/option-(((false)|(true))-)?/, ""), saved = (function web_selectValues_saved() {
                let c = 0;
        ...
        }, selectValues = function web_selectValues(node) {
            let b = 0;
            const opts = node.getElementsByTagName("option"), olen = opts.length, name = node.getAttribute("id").replace("option-", ""), saved = (function web_selectValues_saved() {
                let c = 0;
                if (name === "format") {
                    return "html";
                }
    ...
    { // evaluate query string of the URI for option assignment
        const params = (location.href.indexOf("?") > 0)
            ? location.href.split("?")[1].split("&")
            : [], assignValue = function assignValue(object, name, value) {
        ...
        if (params.length > 1) {
            let aa = params.length, bb = 0, lex = "", pair = [];
    ...
    if (typeof ace === "object") {
        ...
        if (location.href.indexOf("ace=false") > 0) {
            ...
            const div = document.createElement("div"), parent = input.parentNode.parentNode, attributes = input.attributes, dollar = "$", len = attributes.length;
            let a = 0, edit = {}, textarea;
            do {
    ...
    if (location.href.indexOf("//localhost:") > 0) {
        ...
        if (location.href.indexOf("scrolldown") > 0) {
            const body = document.getElementsByTagName("body")[0], el = document.getElementById("data");
            handler();
