...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['link'] = 'https://www.obejor.com.ng/orderhistory';
// 		$data['link'] = $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_info['order_id'];
        ...
        /*$mailb = new PHPMailer;
        //$mailb->SetLanguage( 'en', 'phpmailer/language/' );
        $mailb->isSMTP();
        ...
        //$mailb->Host = 'smtp.google.com';
        //$mailb->Port = 465;
        $mailb->Port = 587;
        $mailb->Username = 'sales@obejor.com'; //@gmail.com';
        //$mailb->Username = 'obejorcom@gmail.com'; //@gmail.com';
        $mailb->Password = 'Google22.@';
        $mailb->setFrom('sales@obejor.com', 'Obejor');
        $mailb->addReplyTo('sales@obejor.com', 'Obejor');
        $mailb->addAddress($order_info['email']);
        //$mailb->addCC("obejorbusiness@gmail.com");
        $mailb->addBCC("icisystemng@gmail.com");
        $mailb->Subject = html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8');
        $mailb->isHTML(true);
        $mailb->Body = $this->load->view('mail/order_add', $data);
        
        $mailb->send();*/
        
        //added 19/02/2020 ici
		$bmessg = $this->load->view('mail/order_add', $data);
        ...
        //****end************************//
			...
			$order_seller = $this->model_extension_purpletree_multivendor_sellerorder->getOrderSeller($order_info['order_id']); //($order_id);
			...
			if($order_seller){
				foreach($order_seller as $orderseller){
					...
					// HTML Mail
					$datac['text_received'] = $this->language->get('text_received');
					$datac['text_order_id'] = $this->language->get('text_order_id');
					$datac['text_date_added'] = $this->language->get('text_date_added');
					$datac['text_order_status'] = $this->language->get('text_order_status');
					$datac['text_product'] = $this->language->get('text_product');
					$datac['text_total'] = $this->language->get('text_total');
					$datac['text_comment'] = $this->language->get('text_comment');
					
					$datac['order_id'] = $order_info['order_id'];
					$datac['date_added'] = date($this->language->get('date_format_short'), strtotime($order_info['date_added']));
					...
					if ($order_status_query->num_rows) {
						$datac['order_status'] = $order_status_query->row['name'];
					} else {
						$datac['order_status'] = '';
					}
					...
					$datac['products'] = array();

					$order_products = $this->model_extension_purpletree_multivendor_sellerorder->getSellerOrderProducts($order_info['order_id'], $orderseller['seller_id']);
					...
					$datac['vouchers'] = array();
					
					$order_vouchers = $this->model_extension_purpletree_multivendor_sellerorder->getOrderVouchers($order_info['order_id']);
					...
					$order_totals = $this->model_extension_purpletree_multivendor_sellerorder->getOrderTotals($order_info['order_id'], $orderseller['seller_id']);
					...
					$datac['comment'] = strip_tags($order_info['comment']);*/
					...
					$mail->setText($this->load->view('mail/order_alert', $datac));
					$mail->send();*/	
					
                    /*$mailc = new PHPMailer;
                    //$mailc->SetLanguage( 'en', 'phpmailer/language/' );
                    $mailc->isSMTP();
                    $mailc->SMTPSecure = 'tls';
                    //$mailc->SMTPSecure = 'ssl';
                    $mailc->SMTPAuth = true;
                    $mailc->Host = 'mail.obejor.com';
                    ...
                    //$mailc->Port = 465;
                    $mailc->Port = 587;
                    $mailc->Username = 'sales@obejor.com'; //@gmail.com';
                    $mailc->Password = 'Google22.@';
                    $mailc->setFrom('sales@obejor.com', 'Obejor');
                    $mailc->addReplyTo('sales@obejor.com', 'Obejor');
                    $mailc->addAddress($seller_detail['email']);
                    $mailc->addBCC("icisystemng@gmail.com");
                    $mailc->Subject = html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8');
                    $mailc->isHTML(true);
                    $mailc->Body = $this->load->view('mail/order_alert', $datac);
                    $mailc->send();
	...
	public function edit($order_info, $order_status_id, $comment) {
		...
		//$mail->setText($this->load->view('mail/order_edit', $msg));
		//$mail->setText($msg);
		$mail->send();*/
		
        //added 19/02/2020 ici
		$bmessg = $this->load->view('mail/order_edit', $data);
		$subject = html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8');
		$cemail = $order_info['email'];
		$postdata = "email=" . $cemail . "&body=" . $bmessg . "&subject=" . $subject;
        ...
        $mailb->Subject = html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8');
        $mailb->isHTML(true);
        $mailb->Body = $this->load->view('mail/order_edit', $data);
        $mailb->send();*/
        
        //****end************************//
        
        //added 07/01/2020 ici
		/*$bmessg = "Your order, No: " . $order_info['order_id'] . " on Obejor.com. New status: " . $data['text_order_status'] . ". Thank you for shopping at Obejor.";
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
			...
			if($order_seller){
			    ...
			    foreach($order_seller as $orderseller){
			        $resorder_products = $this->model_extension_purpletree_multivendor_sellerorder->getSellerOrderProducts($order_info['order_id'], $orderseller['seller_id']);
			        $sellers = $this->model_extension_purpletree_multivendor_sellerusers->getStoreUsers($orderseller['seller_id']);
			        
			        $productids[] = array(
			            "product_id" => $resorder_products['product_id'],
			            "product_name" => $resorder_products['name'],
			...
			foreach ($order_products as $order_product) {
			    ...
			    // Seller mail new code added 16/01/2020
			    $proid = "";
			    $st_name = "";
			    if($order_seller){
			        foreach($productids as $pids){
			            if($pids['product_id'] == $order_product['product_id']){
			                $proid = $pids['product_id'];
			                $st_name = $pids['store_name'];
			            }
			        }
			    }
            ...
            //added 04/03/2020 ici
		    $bmessg = $this->load->view('mail/order_alert', $data);
